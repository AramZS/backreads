...
exports.convertDateToLocalString = function (dateObj) {
	var dateSet = dateObj
		.toLocaleString("en-US", { timezoneName: "ET" })
		.split(",")[0]
		.split("/");
...
exports.resolveLinks = async function (linkSet, aCallback, ua) {
	...
	let user_agent_windows =
		"Mozilla/5.0 (Windows NT 10.0; Win64; x64) " +
		"AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.149 " +
		"Safari/537.36";
	let user_agent_macbook =
			...
			try {
				...
				if (r.ok) {
					...
					try {
						...
						let description = "";
						try {
							description = dom.window.document.querySelector(
						...
						if (exports.collectableLink(url)) {
							if (aCallback) {
								let check = await aCallback(r.url, {
									title: "",
									description: "",
								});
							}
					...
					if (
						cloudflareBlock.test(linkObj.title) ||
						fourOhThreeBlock.test(linkObj.title) ||
						humanCheckBlock.test(linkObj.title)
					) {
					...
					console.log(
						"Attempt to resolve link failed for ",
						link,
						"with ua",
						user_agent_desktop,
